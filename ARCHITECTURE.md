...
The Piece class captures the active piece. A piece is a [tetromino](https://en.wikipedia.org/wiki/Tetromino). The different kinds of pieces can be found in the PieceKind enum and the layouts of each piece can be found in the gfx directory of the project.
...
The well is captured by the aptly named `Well` class. The underlying data type backing the well is a two-dimensional grid, similarly to a piece's layout. Row/col pair (0, 0) corresponds to bottom-lft corner in the well.

## Rendering

The BlockyPanel class is responsible for rendering the game to the screen. The `paintComponent` method performs the actual drawing by, first, rendering panel and, then, the active piece.